    ...
    return (
        <div className="bg-white rounded-xl shadow-sm border border-stone-100 overflow-hidden hover:shadow-md transition-shadow relative group">
            <Link
                ...
                className="absolute top-2 left-2 z-10 p-1.5 bg-white/80 backdrop-blur-sm rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:bg-white text-stone-600"
                onClick={(e) => e.stopPropagation()}
            >
                <Edit2 size={14} />
            </Link>

            <Link to={`/inventory/${tea.id}`} className="block">
